...
from DCPoint.models.utils import sample_and_group
import torch.optim.lr_scheduler as lr_sched
from torch.utils.data import DataLoader, DistributedSampler
from torchvision import transforms

import DCPoint.data.data_utils as d_utils
from DCPoint.data.ModelNet40Loader import ModelNet40Cls
...
def get_graph_feature(x, k=20, idx=None):
    ...
    feature_1 = feature.view(batch_size, num_points, k, num_dims)
    x = x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, k, 1)

    feature = torch.cat((feature_1 - x, x), dim=3).permute(0, 3, 1, 2).contiguous() ###(relative distance, absolate location)
    return feature
...
class DGCNN(nn.Module):
    ...
    def forward(self, pc):
        ...
        x = get_graph_feature(pc, k=self.k)
        x = self.conv1(x)
        ...
        pc = pc.reshape(batch_size, num_points, -1)
...
class TargetNetwork_DGCNN(DGCNN):
    ...
    def forward(self, pointcloud):
        ...
        z: projection
        """
        y, l = super(TargetNetwork_DGCNN, self).forward(pointcloud)
        z = self.projector(y)
        return y, l, z
...
class OnlineNetwork_DGCNN(TargetNetwork_DGCNN):
    ...
    def forward(self, pointcloud):
        ...
        """
        y, l, z = super(OnlineNetwork_DGCNN, self).forward(pointcloud)
        qz = self.predictor(z)
        return y, l, z, qz
